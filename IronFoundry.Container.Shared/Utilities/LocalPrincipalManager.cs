    ...
    public class LocalPrincipalManager : IUserManager
    {
            ...
            using (var context = new PrincipalContext(ContextType.Machine))
            {
                ...
                try
                {
                    do
                    {
                        try
                        {
                            rvPassword = Membership.GeneratePassword(8, 2).ToLowerInvariant() +
                                         Membership.GeneratePassword(8, 2).ToUpperInvariant();
        ...
        private void AddUserToGroup(PrincipalContext context, string groupName, UserPrincipal user)
        {
            ...
            using (var group = InvokeSearcherWithRetries(searcher))
            {
                if (group == null)
                {
                    throw new ArgumentException(string.Format("The specified group '{0}' does not exist.", groupName),
                        "groupName");
                }
